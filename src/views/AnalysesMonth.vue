  ...
  <v-container>
    <template v-if="loading">
      <v-layout row justify-center>
        <v-progress-circular :size="70" class="my-4" indeterminate color="primary"></v-progress-circular>
      </v-layout>
    </template>
    <template v-else>
      <template v-if="monthAnalysis===null">Nenhum dado para exibir.</template>
      <template v-else>
        <v-layout :row="$vuetify.breakpoint.lgAndUp" :column="$vuetify.breakpoint.mdAndDown">
          <v-flex xs12 mb-3 lg4>
            <v-card class="pa-3 app-card">
              <salary-highlight
                :total="salaryTotal"
                :actual="sumComposition(monthAnalysis.salariesTotal)"
              >Total Gasto</salary-highlight>
            </v-card>
          ...
          <v-flex xs12 mb-3 lg4 :mx-4="$vuetify.breakpoint.lgAndUp">
            <v-card class="pa-3 app-card">
              <p
                class="body-2 text-uppercase grey--text text--darken-2"
              >Total por regime de contratação</p>
              <div v-for="(item, i) in monthAnalysis.salariesByType" :key="i" class="mb-4">
                <salary-highlight
                  :total="salaryTotal"
                  :actual="sumComposition(item.composition)"
                  color="blue"
                >{{item.text}}</salary-highlight>
              </div>
            </v-card>
          </v-flex>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getMonthReport", this.params).then(report => {
      this.salaryTotal = this.sumComposition(report.salariesTotal);
      this.monthAnalysis = report;
      this.loading = false;
    });
  }
};
